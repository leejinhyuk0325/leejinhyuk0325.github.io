...
import { getShareCount } from "@/utils/serials";

export default function SerialCard({ serial, variant = "default" }) {
  ...
  const variantStyles = {
    ...
    urgent:
      "bg-gradient-to-br from-rose-50 via-pink-50 to-red-50 border border-rose-200/50",
  ...
  const tagList =
    serial.tagList || serial.tags.split(" ").filter((tag) => tag.trim());
  ...
  return (
    ...
    >
      <Link href={`/detail/${serial.id}`} className="block cursor-pointer">
